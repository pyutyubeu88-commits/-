...
### [2026-07-31] 강남서초 당근모임 "기브넷(GiveNet)" 설계 완료
- 신규 사업: 당근마켓 "모임" 기능으로 강남·서초 3040 상호부조 네트워크 개설 예정 (모임장=권용준)
- 문서: `givenet-meetup-manual.md` (운영 매뉴얼 v1.0, 전체 설계 완료본)
- 모임명: 강남서초 기브넷 | 맛집·취미모임 (카테고리: 사교/친목)
- 핵심 정체성: "도움 주고받는 관계"가 본질, 맛집탐방/취미모임은 수단. 미혼·커플표기 등은 비공개 심사·내부공지로만 처리(공개글엔 미노출) — 당근 소개팅목적 모임 제재 리스크 회피
- 확정된 당근 모임 노출 알고리즘(공식 FAQ): 관심사 적합성+활동점수(글/일정/댓글)+거리
- 외국인 환영(운영진 한/영/일 가능), 장소는 사업자등록+온라인검증 가능한 곳이면 제한 없음(컨설팅 클라이언트 업체는 1순위 우선 채널일 뿐 배타적 아님 — 클라이언트 계속 느는 게 자산)
- 안전프로토콜·강퇴절차(즉시강퇴 vs 경고후강퇴, 부운영진 교차확인) 문서화 완료
- 억만장자팀 스킬로 수익화 로드맵 검토(Godin 포지셔닝/Brunson 가치사다리/Gary Vee 콘텐츠 40·30·20·10/Kennedy 배타성 경고) → 매뉴얼 9번에 반영
- [2026-07-31 수정] 당근 정책 사전문의는 불필요로 판단 확정: 사용자가 "이미 그렇게 운영되는 모임 많다"(시장 선례)고 판단 → 문의 단계를 실행 필수 관문에서 제거, 로컬 제휴(수익화 2단계)도 즉시 병행 가능으로 변경. 문의 템플릿은 매뉴얼에 참고용(접이식)으로만 남김
- 컨설팅 클라이언트 리스트 지속 확장 중(역삼효태권도장/이자카야우규역삼점/다물도신사점 외 더 있음) → 도도넷 장소풀·리드채널로 계속 연동 예정
- [2026-07-31] 대표이미지 1차 시도: HTML→헤드리스 크로미움 렌더(1080x1080, givenet-img/cover.html) — 사용자가 "허접하다" 평가, GPT Image 2 프롬프트로 전환 요청
- [2026-07-31] GPT Image 2용 프롬프트 3회 반복 설계: ①스카이라인+네트워크라인 버전 → 사용자가 실제 결과물(롯데타워=잠실, 강남서초 아님) 보여주며 지적 → ②디자인전문가+트렌드세터 리뷰로 랜드마크 오류·라운디드폰트·당근마스코트 차용·클리셰 네트워크비주얼 지적 후 개선판 제공 → 사용자가 재생성 결과물 보여주며 "인간미 중시로 방향 전환" 요청 → ③사람 중심(따뜻한 조명·사람들이 돕는 제스처·골든아워) 프롬프트로 완전 피벗
- [2026-07-31] ★모임명 재변경: 기브넷 → 도도넷 → 도도남 도도녀 (최종)★ — 사용자가 "도도넷 말고 도도남 도도녀를 직접 넣자"고 확정. "도도=도움주고 도움받고"의 줄임말. 문서 내 짧게 지칭할 땐 "도도"
- ⚠️리스크 재확인 필요: "도도남 도도녀"를 나란히 쓰면 "훈남훈녀"류 소개팅/미팅 캐릭터 지칭 어투와 유사해져서, 단어 하나("도도넷")였을 때보다 소개팅 목적 모임으로 오인될 가능성이 더 커짐. 소개글 최상단 반전 카피("도도남? 도도녀? 새침한 거 아니에요")로 상쇄하는 선에서 사용자가 감수하고 진행하기로 함(8번 정책리스크 항목과 연결됨, 신고 누적 시 재검토 필요)
- [2026-07-31] ★대표 이미지 최종 확정★: GPT Image 2로 생성한 "사람 중심+여름+도도남 도도녀" 버전이 텍스트 정확도·인간미·브랜드컬러 전부 통과. 매뉴얼 11번에 최종 프롬프트 전문 기록.
- [2026-07-31] ★세션 마무리: 당근모임 기획 전체를 `givenet-meetup-manual.md` v1.3 단일 문서로 완결★ (1.정체성 2.모임명 3.소개글 4.가입심사 5.장소기준 6.안전프로토콜 7.강퇴절차 8.정책리스크 9.수익화로드맵 10.실행우선순위 11.대표이미지프롬프트 12.개설직전 체크리스트). 사용자가 "이제부터 클로드 코드 터미널 사용 예정"이라고 밝힘 → 이 세션(웹/CCR)은 여기서 핸드오프, 다음은 터미널 세션이 이 문서 하나만 읽으면 전체 맥락 이어받을 수 있음
- 남은 미완료: 대표이미지 축소 썸네일 가독성 확인(사용자 진행 중) → 그 외엔 실제 당근 앱 개설 실행만 남음
...
### [2026-08-11] 타로카드 하루 마스터 코스 제작
- `tarot-crash-course.html` 신규 작성 — 비전문가용 10레벨 속성 강의 단일 HTML 파일
