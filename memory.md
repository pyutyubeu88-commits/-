...
### [2026-07-29] 신규 프로젝트 논의 — 일본 쇼핑몰 최저가 검색 (아마존재팬/라쿠텐)
- 참석자: 참석자1(발주/의사결정), 참석자2(실무/테스트 담당)
- **목적**: 일본어 검색 키워드(상품 정보)를 입력하면, 아마존재팬 또는 라쿠텐에서 해당 상품을 검색해 "그 상품 자체"의 최저가만 가져오는 자동화
- **핵심 문제**: 아마존재팬·라쿠텐 모두 특정 상품으로 검색해도 관련 상품(다른 옵션·유사상품 등)이 함께 노출됨 → 그중 최저가를 그냥 가져오면 엉뚱한 상품 가격이 잡힘. 반드시 "동일 상품 여부"를 검증한 후 그 안에서만 최저가를 골라야 함
- **한/일 상품코드 차이**: 일본은 ASIN 같은 상품코드가 있어 AI들이 "코드만 있으면 완벽 매칭 가능"이라 하지만, 한국은 그런 코드 체계로 안 팔기 때문에 애초에 ASIN을 안 씀 → 상품명(텍스트) 매칭만으로 해결하는 방법이 필요 (이게 사실상 이 프로젝트의 핵심 과제)
- **매칭 방법론(참석자2 제안)**: 먼저 검색 결과를 전부 긁어온 뒤(스크래핑), "소거법"으로 원하는 상품이 아닌 것들을 걸러내는 방식이 AI 활용 시 오히려 성공 가능성이 높음
- **이미지 매칭 필요성(참석자1)**: 한국에서 파는 판매자의 상품 URL을 입력으로 주면, 그 URL 안의 상품 이미지를 라쿠텐/아마존재팬 검색 결과 이미지와 대조 → 정확히 매칭되는 상품들 중에서만 최저가 선택. 목적은 "한국 판매가보다 싸게 팔 수 있는지" 확인
- **결론/원칙**: 절대 다른 상품(오매칭) 가격을 가져오면 안 됨 — 오매칭 리스크에 민감한 프로젝트(사장님 다수가 이용 예정)
- **진행 상태 및 향후 계획**: 프로토타입으로 매칭 가능성 테스트 → 가능성 확인되면 자동화 고도화. 현재는 황선진 프로님과 진행 중이며 담당 시스템/툴은 미확정(사람만 바뀌는 상황). 추후 참석자2가 검증되면 참석자1의 아들에게 실제 운영을 맡기는 방향 논의
- **참고**: 녹취 분량(20분 이하)에 대한 처리 우려가 있었으나, 스크립트(텍스트)만 있으면 분량과 무관하게 처리 가능하다고 정정됨
- 브랜치명(japan-shopping-price-search)과 연결된 신규 프로젝트 — 아직 코드/설계 착수 전, 요구사항 정리 단계
