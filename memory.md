...
### [2026-07-28] ★인생사업소★ 신규 사업 기획 — 전체 요약 (다른 세션/터미널 인계용)
- **계기**: 자주 가는 정형외과 선생님이 권용준에게 "고민 상담 사업 해봐라"고 추천 → 첫원칙 사고로 현실적 방법 탐색
- **핵심 자산**: 13년 보험영업(2014~, 메트라이프→GA→롯데손보·동양생명) + 2019~2025 유사투자자문업 직접 창업(3인팀, 월매출 1억, 유튜브 동시시청 1,000명). 종료 사유는 규제 문제 아니라 트럼프발 시장 불확실성(자연 종료). 이 경력은 **"은근히 배경으로만"** 노출(profile.html 수준) — 절대 전면에 내세우지 않기로 결정됨
- **사업명**: "인생사업소" (후보였던 "용준이형 라인"은 여성 고객 대부분인데 남성 손윗사람 호칭이라 기각, "형/오빠" 등 친밀 호칭은 상담 중 연애 얘기까지 다루는 구조상 경계 문제로 배제 → 중립 호칭 채택)
- **타겟/채널 — 반드시 트랙 구분할 것**:
  - **트랙A (지인 네트워크, 사업화 시작 기준)**: 몽슈슈 · S&H왁싱(압구정왁싱샵) · 포차미루 — 3인 전원 원래 개인적 친분으로 AI컨설팅 해준 관계, 제3자 개입 없음. 실제로 몽슈슈는 사업컨설팅 하다가 연애상담까지 무료로 자연스럽게 해준 전례 있음(수요 사전검증됨)
  - **트랙B (강남구 매칭, 전면 보류)**: 이자카야우규·다물도 — 강남구+상상우리 공식 배정 무료 컨설팅 클라이언트. 여기다 개인 유료 서비스 팔면 이해충돌(세금으로 운영되는 공식관계를 개인사업 영업기회로 이용) 리스크 있음. **상상우리 정책 확인 전까지 절대 먼저 권유 금지**
- **오퍼 구조 (4단 가격 사다리)**: 0.무료 첫대면(30분,의도적 미완결) → 1.입문 90분 15~20만원 → 2.핵심 월1회×4주 월50~70만원 → 3.프리미엄 분기 200~300만원
- **⚠️ 시뮬레이션에서 확인된 핵심 리스크**: 0단계를 "60분 자유롭게 다 들어주기"로 하면 그 자리에서 이미 후련해져서 유료전환 유인이 사라짐(몽슈슈 패턴 재현 확인). 반드시 **30분 컷 + 의도적 미완결**("오늘 들어보니 [문제]가 얽혀있는데 30분으론 부족해요")로 진단하고 끊어야 함
- **트랙A 전환 프로세스**: 지인 소개 요청("주변에 이런 얘기 상대 필요한 사람 있어?") → 카톡 카드 전달 → 0단계(30분 미완결) → 1단계(90분 유료) 제안
- **AI컨설팅 신규고객 3회차 전환 스크립트** (트랙A 전용, 트랙B 절대 금지): "세 번 뵈면서 사업 얘기만 한 게 아니라 개인 고민도 자연스럽게 나눴잖아요. 저 따로 그런 것도 봐드리고 있어요 — '인생사업소'라고. 컨설팅이랑 별개로 다음에 시간 잡고 얘기 나눠볼까요?"
- **법적 가드레일**: ①"상담" 대신 "멘토링"(심리상담사 자격 이슈 회피) ②"투자자문"처럼 안 보이게 설계 ③경력 노출 최소화 ④호칭 중립
- **확장성 냉정한 진단**: 지금 구조(1:1 대면+소개기반)로는 본업급 스케일 불가능(시간 파는 모델, 하루 24시간 천장). 유사투자자문업 때 월1억은 유튜브 라이브(1:다) 구조였기 때문. 본업급 가려면 나중에 핵심고객 5~10명 쌓인 후 그룹세션/워크숍으로 콘텐츠화 필요(e북 사업 패턴 재활용 가능)
- **생성된 파일** (`insaengsaeopso/` 폴더, 전부 커밋됨):
  - `card.html`/`card.png` — 카톡 공유용 소개카드 (1080x1350, 그린테마+골드포인트, 강남구 공식 명함 이미지를 신뢰배지로 삽입, 증명사진 헤드샷 사용)
  - `business-plan.html` — 전체 사업계획 정리본 (트랙A/B 구분, 오퍼 사다리, 법적가드레일, 확장성 로드맵, 다음액션 체크리스트 전부 포함) ★가장 중요한 참조 파일★
  - `profile-id-sq.jpg`, `business-card-clean.jpg` 등 이미지 소스
  - 폰트 이슈: Google Fonts 네트워크 호출이 프록시에서 SSL 실패 → Noto Sans CJK KR 로컬 설치(~/.fonts) 후 font-family 직접 지정으로 해결. **주의: 컨테이너 재시작하면 폰트 재설치 필요**(ephemeral)
- **다음 액션 (미완료)**: 몽슈슈·S&H·미루 3인에게 소개 요청 던지기 → 실제 소개 성사 여부 관찰 → 소개 들어오면 0→1단계 프로세스 실행 → 결과 가지고 가격/오퍼 재검증
- **보류 중**: 상상우리에 트랙B 관련 정책 문의 (아직 안 함)
