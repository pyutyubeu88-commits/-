...
# ⚠️ 공식 로고/심볼은 회사 승인 자산만 사용 — AI가 흉내낸 짝퉁 로고는 상표권/컴플라이언스 위험.
AGENT_NAME = "롯데손해보험 전속설계사 권용준"
AGENT_TEL  = "010-6783-2588"      # 설계사 연락처
OPTOUT_TEL = "080-870-5377"       # 무료수신거부 번호 (본인 발급)
...
MOTIF = {
    1: "초여름 햇살 드는 창가에 시원한 음료 한 잔(얼음/물방울)과 싱그러운 초록 화분. "
       "맑고 청량한 초여름 느낌의 3D 렌더 스타일 일러스트를 상단에 크게.",
...
def build_prompt(spec: dict) -> str:
    ...
    if spec["is_ad"]:
        ad_line = (
            ...
            f"맨 하단에 작은 회색 글씨로 '무료수신거부 {OPTOUT_TEL}' 표기.\n"
        )
    big_line = (
    ...
    return (
        ...
        f"· 맨 하단: 좌측에 작은 회색 글씨 '롯데손해보험' (텍스트만, 그 앞/뒤에 어떤 아이콘·심볼·도형·원형 뱃지도 없음), "
        f"우측에 '{AGENT_NAME} {AGENT_TEL}'\n"
        f"{ad_line}"
        "\n⚠️⚠️ 로고·심볼 완전 금지 (매우 중요) ⚠️⚠️\n"
        "회사명 '롯데손해보험' 텍스트 앞/뒤/옆에 어떤 형태의 그래픽도 넣지 말 것:\n"
        "- 빨간 원형·타원·뱃지 → 금지\n"
        "- 'ㄴ' 'L' 'ㄹ' 'e' 모양 심볼 → 금지\n"
        "- 회사 로고처럼 보이는 모든 아이콘/엠블럼/마크 → 금지\n"
        "- '롯데손해보험' 글자 자체 외의 장식 요소 → 금지\n"
        "회사명은 반드시 '롯데손해보험' 텍스트 6글자만으로 표기. 그래픽 요소 일절 없음.\n"
