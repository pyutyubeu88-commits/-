...
| 단계 | 예시 요청 | 처리 방식 | 상태 |
|---|---|---|---|
| 1. 컷 편집 | "이 영상 편집해줘. 같은 말 여러 번 한 데는 마지막 것만 써. 1.1배속으로." | `scripts/cut_edit.py` (whisper 받아쓰기 → 재테이크는 자동 컷, 무음 구간은 `<output>.silence_candidates.json`으로 제안만 — 실제로 자르려면 `--apply-silence-cuts` 명시) | 로컬에서 바로 가능. 무음 트리밍은 기본 비활성, 제안 목록을 사람이 검토한 뒤 승인 |
| 2. 화면 리프레임 | "가로 찍은 거 세로로. 얼굴 따라가면서 잘라." | `scripts/reframe_vertical.py` (ffprobe로 종횡비 확인 → 이미 9:16에 가까우면(0.55~0.58) **기본 스킵**하고 원본 통과, 가로 촬영본 등 다를 때만 opencv 얼굴 검출 → ffmpeg 동적 크롭. `--force`로 강제 가능) | 정적 인터뷰 촬영은 기본 스킵됨 — 세로 촬영 SOP를 따르면 이 단계 자체가 발동 안 함. 얼굴추적이 실제로 도는 케이스만 정확도 확인 필요 |
| 3. 상단 질문 배너 + 로고 워터마크 | "위에 질문 고정으로 띄우고, 로고 박아줘." | `scripts/add_branding.py` (질문 배너는 ffmpeg drawtext/drawbox, 로고는 `--logo`로 실제 로고 PNG를 overlay — 텍스트 재구성 아님. 클라이언트별 로고/강조색/브랜드명은 `--client-config clients/<이름>/config.json`으로 주입, 하드코딩 안 함) | 로컬에서 바로 가능, 한글 폰트 파일(.ttf) 경로 필요. 스마일뷰는 `clients/스마일뷰/config.json`(accent `#225FEC`) 사용 |
...
| 6. 없는 장면 대체 | "이 대사에 붙일 장면 없어. B-roll 끼워줘." | `scripts/insert_broll.py` (직접 찍은 컷 또는 무료 스톡 클립을 `--at`/`--duration`으로 지정 구간에 덮어씌움, 나레이션은 안 끊김) | **로컬 무료** — AI 생성 대신 진짜 클립 사용 |
| 6-Q. B-roll 큐시트 자동 매칭 | "대본 이 문장 나올 때 이 클립 끼워줘 — 몇 초인지는 몰라." | `scripts/resolve_broll_cues.py` (큐시트의 문구를 whisper word-timestamp 전사와 **정확 부분문자열 매칭**으로 대조해 시작 시각을 찾고 `insert_broll.py`를 그 타임스탬프로 자동 실행) | 로컬 무료. **매칭 실패 시 조용히 넘어가지 않고 즉시 에러** — 문구를 whisper 전사 결과에서 그대로 복사했는지 확인하거나 `insert_broll.py`로 수동 지정 |
| 7. 효과음 | "효과음 넣어줘. 목소리보다 작게, 자막 뜨는 순간에 맞춰." | `scripts/add_sfx.py` (무료 SFX 파일을 타이밍 맞춰 ffmpeg로 합성) | **로컬 무료** — AI 생성 대신 무료 라이브러리 소스 |
| 7-B. 인스타 캡션 생성 | "캡션이랑 해시태그도 뽑아줘." | `scripts/generate_caption.py` (질문+전사 텍스트 → 인스타 게시글 캡션/해시태그 초안, 의료광고법 금칙어 필터 내장) | 로컬에서 바로 가능 — 영상과 별개로 `.caption.txt` 산출 |
| 8. 최종 검수 | "결과물 폴더 열어주고, 완성본 다시 받아 적어서 대본이랑 맞는지 봐줘." | `scripts/qc_check.py` (whisper 재검증 → 대본 대조) | 로컬에서 바로 가능 — **매 작업 마지막에 반드시 실행** |

파이프라인 순서:
**컷편집 → 리프레임 → 브랜딩(정적 또는 모션) → B-roll 삽입 → 화면 그래픽 → 효과음 → 자막 → 자막 강조 모션 → 인스타 캡션 생성 → QC**.
...
- **하단 자막**: **박스 없음** — 흰 굵은 글씨 + 두꺼운 검정 테두리(outline)만, 최대 2줄,
  형광펜/그라데이션 없음. `reference_frames/f_01.png`, `f_03.png`, `f_10.png`(레퍼런스 프레임
  실측 캡처)를 직접 열어보면 자막 글자 뒤로 배경 진료실이 그대로 비치는 게 보인다 — 불투명 박스가
  아니라는 물증. **한때 "불투명 검정 박스가 맞다"고 여기 적혀 있던 적이 있었는데 오판이었다** —
  프레임을 다시 실측해서 정정함. `default_style.json`은 `korean_border_style: 1`(아웃라인 모드,
  박스 아님) + `korean_outline: 8`(두꺼운 테두리)로 맞췄고, `make_captions.py`의 ASS 스타일 라인도
  BorderStyle을 3(박스)에서 1(아웃라인)로 고쳤다
